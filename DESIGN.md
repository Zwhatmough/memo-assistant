...
| Citation verification — M2 failure analysis | Final verification rate: 220/254 (86%), matching Zak's manual 89% check. 34 unverifiable facts fall into three structural categories, not model hallucination: (1) **Infographic/KPI pages** (p4, p19-21): designed multi-column graphic pages where pdfplumber scrambles column order — even 6-word fragment matching fails; (2) **Two-column body text** (p3-6, p6, p11-14): Chair/CEO statement uses two-column layout; pdfplumber interleaves columns, splitting contiguous excerpts across non-adjacent text positions; (3) **Model-assembled table excerpts** (p23-25): model joined table header + data row cells that are not contiguous in linearised extraction order. All 34 are believed correct facts; the limitation is string-matchability, not accuracy. Mitigation: `validate.py` auto-corrects page-drift (7 auto-corrections found), excludes the 34 from downstream stages as "unverifiable" rather than discarding them entirely — they are logged in `validated_facts.json` with `citation_status: unverifiable`. Two-tier matcher implemented: exact substring first, then 6-word fragment fallback for multi-column pages. | Confirmed 14 Jul 2026 |
| Generalisation principle | All company adaptations must be generic mechanisms — configurable or auto-detected, never hardcoded to a specific company — such that adding a new company requires only a `config.yaml` entry and a company-specific checks registry entry, with no changes to pipeline logic. Specifically: (1) Section filter uses `section_filter.py` heading-taxonomy matching with per-company `heading_variants` and `fallback_pages` in `config.yaml`; (2) Finance cross-checks are split into `UNIVERSAL_CROSS_CHECKS` (any company, always run) and `COMPANY_CROSS_CHECKS[company_id]` (company-specific segment logic, loaded by `company_id` from config); (3) All prompts are company-agnostic — company name and document metadata are injected at runtime, not hardcoded into prompt strings. The prompt improvement strings introduced in V2 (direct-traffic moat, EPS divergence) are phrased as conditional questions ("if the facts support them"), so they produce nothing if those patterns don't exist in a second company's data. Classification taxonomy terms (RISK_REGISTER_TAXONOMY in classify.py) are drawn from ISO 31000/COSO ERM/TCFD and apply to any company. Validated against Auto Trader. Generalisability to be confirmed with second company (Greggs PDF pending). | Confirmed 17 Jul 2026 |

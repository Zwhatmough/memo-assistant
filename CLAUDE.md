...
**V2 improvement round — implemented, run pending.** Three changes implemented in `classify.py` and `memo.py` (see BUILD_LOG.md). V2 run blocked by API monthly limit (resets 2026-08-01). When the API is available, run: `python3 classify.py --out output/v2/classified_facts.json` then `python3 memo.py --classified output/v2/classified_facts.json --memo-out output/v2/memo.md --register-out output/v2/evidence_register.json`. Score V2 with: `python3 eval/run_eval.py --memo output/v2/memo.md --classified output/v2/classified_facts.json`. Report V1 and V2 side by side in `eval/results.md §5`. Note: V2 changes are designed for generalisability but have only been tested on Auto Trader — validate on a second company before treating them as settled improvements.

...
**V3 improvement round — complete (automated portion, 17 Jul 2026).** Structural Section 6 enforcement: `build_risk_skeleton()` in `classify.py` derives disclosed risk categories deterministically from `risk_disclosures` facts; stored in `classified_facts.json`. Synthesis prompt updated with `RISK COVERAGE MANDATE` (explicit risk framing required per category). `memo.py` generates one `###` sub-section per skeleton category; `validate_risk_coverage()` post-generation validator fails if any skeleton keyword absent from Section 6. V3 outputs at `output/v3/`. Automated results: fact recall 18/20 (90%), citation accuracy 16/18 (88%), risk coverage 10/10 (100%) — R-05 now has genuine "cyber"/"security" keyword matches; R-10 has "third"/"reliance" matches (framing mandate effective). Also fixed two number-validator false positives (`k`-suffix numbers, `100%` threshold reference). Manual verification pending: `eval/v3_scoring_addendum.md` — Zak to fill in risk verdicts only. V3 changes have only been tested on Auto Trader.
